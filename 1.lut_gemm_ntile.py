...
N_Tile = 32
@T.prim_func
def main_mTile(TABLE: T.Buffer(TABLE_shape, dtype_table), B: T.Buffer(B_shape, dtype_b), C: T.Buffer((M, N), dtype_table)):
    ...
    with T.Kernel(M, T.ceildiv(N, N_Tile), threads=threads) as (bx, by):
        for n in T.serial(N_Tile):
            ...
            for k in T.serial(K // GROUP):
                query[0] = B[by * N_Tile + n, k]
                accum_res[0] += TABLE[bx, k, query[0]]
            C[bx, by * N_Tile + n] = accum_res[0]
...
mod, params = tl.lower(main_mTile)
