...
# -1 or 1
weight_int1 = torch.randint(0, 2, (K, N), dtype=torch.int8, device="cuda")
weight_int4 = torch.zeros((K, N // GROUP), dtype=torch.int8, device="cuda")
...
for k in range(K):
    for n in range((N // GROUP) // 2):
        weight_8bit = 0
        for i in range(2):
            weight_8bit |= (weight_int4[k, n * 2 + i] << (4 * i))
