...
@T.prim_func
def main_nTile_kTile(TABLE: T.Buffer(TABLE_shape, dtype_table), B: T.Buffer(B_shape, dtype_b), C: T.Buffer((M, N), dtype_table)):
    accum_res = T.alloc_fragment((N_Tile,), dtype_table, "local")
    query = T.alloc_fragment((1,), "int8", "local")
    with T.Kernel(M, T.ceildiv(N, N_Tile), threads=threads) as (bx, by):
        for n in T.serial(N_Tile):
            accum_res[n] = 0
        for ko in T.serial(((K // GROUP) // K_Tile)):
            for n in T.serial(N_Tile):
                for ki in T.serial(K_Tile):
                    query[0] = B[by * N_Tile + n, ko * K_Tile + ki]
                    accum_res[n] += TABLE[bx, ko * K_Tile + ki, query[0]]
        for n in T.serial(N_Tile):
            C[bx, by * N_Tile + n] = accum_res[n]
...
mod, params = tl.lower(main_nTile_kTile)
